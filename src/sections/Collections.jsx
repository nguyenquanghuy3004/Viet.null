      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div className="max-w-xl">
            <h2 className="text-gold uppercase tracking-widest text-sm mb-4 font-bold">{t('collections.subtitle')}</h2>
            <h3 className="text-5xl md:text-6xl font-serif text-white leading-tight">
              Bốn Kỷ Nguyên <br /> <span className="text-gold italic">Hào Khí Việt</span>
            </h3>
          </div>
          <p className="text-white/70 max-w-sm text-lg leading-relaxed">
            Mỗi thiết kế là một hành trình xuyên thời gian, tôn vinh những giá trị văn hiến và bản sắc dân tộc.
          </p>
        </div>
          ...
          {currentDynasties.map((dynasty, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-8 z-20 group-hover:bg-heritage-black/20 transition-all duration-500">
                ...
                <p className="text-white/90 text-sm mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-y-4 group-hover:translate-y-0 leading-relaxed font-medium">
                  {dynasty.description}
                </p>
                <div className="w-10 h-0.5 bg-gold group-hover:w-full transition-all duration-700"></div>
                <div className="mt-4 text-[8px] text-gold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Nhấn để xem câu chuyện</div>
              </div>

              <div className="absolute top-8 right-8 text-gold/10 text-8xl font-serif select-none pointer-events-none group-hover:text-gold/20 transition-colors">
